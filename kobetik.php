...
function dekle($tablo, $bilgiler) {
    ...
    $sonuc = mysql_query($ekleSQL, $paralel_bag) or die(mysql_error());
...
$sorgu = mysql_query($sorgu, $paralel_bag) or die(mysql_error());
...
function sil($tablo,$kriter) {
  ...
  mysql_select_db($veritabani, $paralel_bag);
  $sonuc = mysql_query($silSQL, $paralel_bag) or die(mysql_error());
  ...
  $guncelleSQL = sprintf("UPDATE %s SET %s WHERE %s",
					 $tablo,
					  $set,
					   $where);
  mysql_select_db($veritabani, $paralel_bag);
  $Sonuc = mysql_query($guncelleSQL, $paralel_bag) or die(mysql_error());
}
...
$sorgu = mysql_query($sorgu . $limit, $paralel_bag) or die(mysql_error());
$totalRows_sorgu = mysql_num_rows($sorgu);
if ($totalRows_sorgu==0) {
	return 0;
} else {
